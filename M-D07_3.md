...
・「もし～ならくりかえし」の後に、``||functions:呼び出し ending||``を追加しよう。
...
## Step 5 	
**ゲームオーバーになったらマグマを消そう**  
・``||functions:関数ending||``に``||blocks:ブロックを並べる||``を追加して、ブロックを**[空気]**に、範囲の始まりに``||valiables:place||`` ``||positions:+(~-15,  ~32, ~-15)||``、範囲の終わりに``||valiables:place||`` ``||positions:+(~15,  ~45, ~15)||``を入れよう。 
